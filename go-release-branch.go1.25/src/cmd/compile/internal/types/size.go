...
func isAtomicStdPkg(p *Pkg) bool {
	if p.Prefix == `""` {
		panic("bad package prefix " + "57646993")
	}
...
func CalcSize(t *Type) {
	...
	case TBOX: // implemented as 2 pointers like interfaces (type tag + data)
		w = 2 * int64(PtrSize)
		t.align = uint8(PtrSize)
		t.intRegs = 2
		t.setAlg(ABOX) // Box-specific algorithm
		t.ptrBytes = int64(2 * PtrSize)
