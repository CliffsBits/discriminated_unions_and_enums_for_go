...
const (
	...
	// Box types
	TBOX     // discriminated union types
...
// BoxType returns t's extra box-specific fields.
func (t *Type) BoxType() *Box {
	t.wantEtype(TBOX)
	return t.extra.(*Box)
}
...
// Box contains Type fields specific to box types.
type Box struct {
	variants []*Type // variant types in the discriminated union
}
...
func (t *Type) setVariants(variants []*Type) {
	// If we've calculated the width of t before,
	// then some other type such as a function signature
	// might now have the wrong type.
	// Rather than try to track and invalidate those,
	// enforce that setVariants cannot be called once
	// t's width has been calculated.
	if t.widthCalculated() {
		base.Fatalf("setVariants of %v: width previously calculated", t)
	}
	t.wantEtype(TBOX)
	t.extra = &Box{variants: variants}
}
...
func (t *Type) cmp(x *Type) Cmp {
	...
	case TBOX:
		// Compare box types by their variants
		tb := t.extra.(*Box)
		xb := x.extra.(*Box)
		if len(tb.variants) != len(xb.variants) {
			return cmpForNe(len(tb.variants) < len(xb.variants))
		}
		for i := 0; i < len(tb.variants); i++ {
			if c := tb.variants[i].cmp(xb.variants[i]); c != CMPeq {
				return c
			}
		}
		return CMPeq
...
func (t *Type) IsPtrShaped() bool {
	return t.kind == TPTR || t.kind == TUNSAFEPTR ||
		t.kind == TMAP || t.kind == TCHAN || t.kind == TFUNC || t.kind == TBOX
}

// HasNil reports whether the set of values determined by t includes nil.
func (t *Type) HasNil() bool {
	switch t.kind {
	case TCHAN, TFUNC, TINTER, TBOX, TMAP, TNIL, TPTR, TSLICE, TUNSAFEPTR:
		return true
	}
	return false
}
...
// NumBoxTypes returns the number of variant types in a box type.
func (t *Type) NumBoxTypes() int {
	if t.Kind() != TBOX {
		base.Fatalf("NumBoxTypes: not a box type")
	}
	return len(t.BoxType().variants)
}

// BoxVariant returns the i'th variant type in a box type.
func (t *Type) BoxVariant(i int) *Type {
	if t.Kind() != TBOX {
		base.Fatalf("BoxVariant: not a box type")
	}
	variants := t.BoxType().variants
	if i < 0 || i >= len(variants) {
		base.Fatalf("BoxVariant: index %d out of range [0, %d)", i, len(variants))
	}
	return variants[i]
}
...
func (t *Type) FieldType(i int) *Type {
	if t.kind == TTUPLE {
		...
		default:
			panic("bad tuple index " + "a2c2541c")
		}
	}
	if t.kind == TRESULTS {
...
// NewBox returns a new box (discriminated union) with the given variant types.
func NewBox(variants []*Type) *Type {
	t := newType(TBOX)
...
func IsDirectIface(t *Type) bool {
	...
	case TBOX:
		// Box types are not direct interface - they have their own representation
		return false
