...
func calculateCostForType(t *types.Type) int64 {
	...
	case types.TSTRING, types.TINTER, types.TBOX, types.TCOMPLEX64, types.TCOMPLEX128:
		cost = 2
...
// EqBox returns the nodes
//
//	s.btab == t.btab
//
// and
//
//	boxeq(s.btab, s.data, t.data)
//
// which can be used to construct box equality comparison.
// eqtab must be evaluated before eqdata, and shortcircuiting is required.
func EqBox(s, t ir.Node) (eqtab *ir.BinaryExpr, eqdata *ir.CallExpr) {
	if !types.Identical(s.Type(), t.Type()) {
		base.Fatalf("EqBox %v %v", s.Type(), t.Type())
	}
	// func boxeq(typ *uintptr, x, y unsafe.Pointer) (ret bool)
	fn := typecheck.LookupRuntime("boxeq")

	stab := ir.NewUnaryExpr(base.Pos, ir.OBTAB, s)
	ttab := ir.NewUnaryExpr(base.Pos, ir.OBTAB, t)
	sdata := ir.NewUnaryExpr(base.Pos, ir.OBDATA, s)
	tdata := ir.NewUnaryExpr(base.Pos, ir.OBDATA, t)
	sdata.SetType(types.Types[types.TUNSAFEPTR])
	tdata.SetType(types.Types[types.TUNSAFEPTR])
	sdata.SetTypecheck(1)
	tdata.SetTypecheck(1)
