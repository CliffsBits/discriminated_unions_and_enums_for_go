...
func assignOp(src, dst *types.Type) (ir.Op, string) {
	...
	// 4. dst is a box type and src is assignable to one of its variants
	if dst.Underlying().Kind() == types.TBOX {
		dstBox := dst.Underlying()
		for i := 0; i < dstBox.NumBoxTypes(); i++ {
			variant := dstBox.BoxVariant(i)
			if types.Identical(src, variant) {
				return ir.OCONVBOX, ""
			}
			// Also allow assignment of convertible types (like untyped constants)
			if op, _ := assignOp(src, variant); op != ir.OXXX {
				return ir.OCONVBOX, ""
			}
		}
		return ir.OXXX, fmt.Sprintf(": cannot use %v as %v in assignment", src, dst)
	}
