...
func tcArith(n ir.Node, op ir.Op, l, r ir.Node) (ir.Node, ir.Node, *types.Type) {
	...
	if dt := defaultType(t); !okfor[op][dt.Underlying().Kind()] {
...
// tcBTab typechecks an OBTAB node.
func tcBTab(n *ir.UnaryExpr) ir.Node {
	n.X = Expr(n.X)
	t := n.X.Type()
	if t == nil {
		n.SetType(nil)
	...
	if !t.IsBox() {
		base.Fatalf("OBTAB of %v", t)
	}
	n.SetType(types.NewPtr(types.Types[types.TUINTPTR]))
	return n
}

// tcBData typechecks an OBDATA node.
func tcBData(n *ir.UnaryExpr) ir.Node {
	n.X = Expr(n.X)
	t := n.X.Type()
	if t == nil {
		n.SetType(nil)
	...
	if !t.IsBox() {
		base.Fatalf("OBDATA of %v", t)
	}
	n.SetType(types.Types[types.TUINT8].PtrTo())
	return n
}
...
func tcUnaryArith(n *ir.UnaryExpr) ir.Node {
	...
	if !okfor[n.Op()][defaultType(t).Underlying().Kind()] {
