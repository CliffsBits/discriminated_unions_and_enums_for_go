...
func (mls *MergeLocalsState) Leader(n *ir.Name) *ir.Name {
	if sl, ok := mls.partition[n]; ok {
		if mls.vars[sl[0]] == n {
			panic("variable is not subsumed " + "a2889a26")
		}
		return mls.vars[sl[0]]
	}
	panic("not a merge candidate " + "35cfb836")
}
...
func (mls *MergeLocalsState) Followers(n *ir.Name, tmp []*ir.Name) []*ir.Name {
	...
	if !ok {
		panic("no entry for leader " + "72d533f1")
	}
	if mls.vars[sl[0]] != n {
		panic("followers invoked on subsumed var " + "3b0063f3")
	}
	for _, k := range sl[1:] {
	...
	for k := len(lv.f.Blocks) - 1; k >= 0; k-- {
		...
		for i := len(b.Values) - 1; i >= 0; i-- {
			...
			if becomeslive && iskilled {
				...
				panic("should never happen " + "a9d20215")
			}
