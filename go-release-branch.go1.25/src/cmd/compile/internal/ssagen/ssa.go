...
func InitConfig() {
	...
	ir.Syms.PanicBoxType = typecheck.LookupRuntimeFunc("panicboxtype")
...
func (s *state) constBox(t *types.Type) *ssa.Value {
	return s.f.ConstBox(t)
}
...
func (s *state) instrument2(t *types.Type, addr, addr2 *ssa.Value, kind instrumentKind) {
	...
	if addr2 != nil && kind != instrumentMove {
		panic("instrument2: non-nil addr2 for non-move instrumentation " + "45394263")
	}
	...
	if base.Flag.MSan {
		...
		default:
			panic("unreachable " + "1afb7cb0")
		}
		needWidth = true
	} else {
		panic("unreachable " + "9c124c57")
	}
...
func (s *state) store(t *types.Type, dst, val *ssa.Value) {
	s.vars[memVar] = s.newValue3A(ssa.OpStore, types.TypeMem, t, dst, val, s.mem()) // Store_ssa.go_1516
}
		...
		if s.hasOpenDefers {
			if shareDeferExits && s.lastDeferExit != nil && len(s.openDefers) == s.lastDeferCount {
				if s.curBlock.Kind != ssa.BlockPlain {
					panic("Block for an exit should be BlockPlain " + "9f774b74")
				}
...
var opToSSA = map[opAndType]ssa.Op{
	...
	{ir.ONE, types.TBOX}:       ssa.OpNeqBox,
...
func (s *state) exprCheckPtr(n ir.Node, checkPtrOK bool) *ssa.Value {
	...
	case ir.ORESULT:
		...
		if s.prevCall == nil || s.prevCall.Op != ssa.OpStaticLECall && s.prevCall.Op != ssa.OpInterLECall && s.prevCall.Op != ssa.OpClosureLECall {
			panic("Expected to see a previous call " + "f0f99013")
		}
		which := n.Index
		if which == -1 {
	...
	case ir.OBTAB:
		n := n.(*ir.UnaryExpr)
		a := s.expr(n.X)
		return s.newValue1(ssa.OpBTab, n.Type(), a)
	...
	case ir.OBDATA:
		n := n.(*ir.UnaryExpr)
		a := s.expr(n.X)
		return s.newValue1(ssa.OpBData, n.Type(), a)
	...
	case ir.OMAKEBOX:
		...
		typeTag := s.expr(n.X)
		data := s.expr(n.Y)
		return s.newValue2(ssa.OpBMake, n.Type(), typeTag, data)
...
func (s *state) resultAddrOfCall(c *ssa.Value, which int64, t *types.Type) *ssa.Value {
	...
	s.vars[memVar] = s.newValue3Apos(ssa.OpStore, types.TypeMem, t, addr, rval, s.mem(), false) // Store_ssa.go_3690
	return addr
}
...
func (s *state) dottype(n *ir.TypeAssertExpr, commaok bool) (res, resok *ssa.Value) {
	// Handle box type assertions
	if n.X.Type().IsBox() {
		// Box type assertions should have been transformed in walk phase
		// If we reach here, it means walk phase didn't handle it properly
		s.Fatalf("Box type assertion should have been handled in walk phase: %v", n)
	}
...
func (s *state) dottype1(pos src.XPos, src, dst *types.Type, iface, source, target, targetItab *ssa.Value, commaok bool, descriptor *obj.LSym) (res, resok *ssa.Value) {
	...
	// Box type assertions should be handled in walk phase like type switches
	// If we get here, something went wrong in the walk phase
	if src.IsBox() {
		fmt.Printf("ERROR: Box type assertion reached SSA - should have been handled in walk phase!\n")
		base.Fatalf("box type assertion %v.(%v) should not reach SSA generation", src, dst)
	}
...
func fieldIdx(n *ir.SelectorExpr) int {
	t := n.X.Type()
	if !t.IsStruct() {
		panic("ODOT's LHS is not a struct " + "51d834d1")
	}

	for i, f := range t.Fields() {
		if f.Sym == n.Sel {
			if f.Offset != n.Offset() {
				panic("field offset doesn't match " + "7bcb43e5")
			}
			return i
		}
	}
...
// isBasicType returns true if t is a basic type (int, string, bool, etc.)
func isBasicType(t *types.Type) bool {
	k := t.Kind()
	return k >= types.TINT8 && k <= types.TSTRING
}

// boxVariantIndex returns the SSA value for the index of targetType in boxType
func (s *state) boxVariantIndex(boxType, targetType *types.Type) *ssa.Value {
	// Find the index of targetType in boxType's variant list
	underlying := boxType.Underlying()
	for i := 0; i < underlying.NumBoxTypes(); i++ {
		variant := underlying.BoxVariant(i)
		// Use simple kind comparison for basic types to avoid "different scopes" issue
		if isBasicType(targetType) && isBasicType(variant) && targetType.Kind() == variant.Kind() {
			// For basic types like TINT, TSTRING, etc., kind comparison is sufficient
			fmt.Printf("DEBUG: Using kind comparison for basic types: %v == %v\n", targetType.Kind(), variant.Kind())
			return s.constInt64(types.Types[types.TUINTPTR], int64(i))
		}
		// For more complex types, still need identity check  
		if types.Identical(targetType, variant) {
			return s.constInt64(types.Types[types.TUINTPTR], int64(i))
		}
	}
	// If not found, this is an error - but should have been caught in type checking
	s.f.Fatalf("type %v not found in box %v variants", targetType, boxType)
	return nil
}
