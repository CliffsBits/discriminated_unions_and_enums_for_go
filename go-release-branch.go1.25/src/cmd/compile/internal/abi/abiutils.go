...
func appendParamTypes(rts []*types.Type, t *types.Type) []*types.Type {
	...
	if t.IsScalar() || t.IsPtrShaped() {
		...
		case types.TBOX:
			return appendParamTypes(rts, synthBox)
		}
	}
	return rts
}
...
func appendParamOffsets(offsets []int64, at int64, t *types.Type) ([]int64, int64) {
	...
	if t.IsScalar() || t.IsPtrShaped() {
		...
		case types.TBOX:
			return appendParamOffsets(offsets, at, synthBox)
		}
	}
	return offsets, at
}
...
func (state *assignState) allocateRegs(regs []RegIndex, t *types.Type) []RegIndex {
	...
	if t.IsScalar() || t.IsPtrShaped() {
		...
		case types.TBOX:
			return state.allocateRegs(regs, synthBox)
		}
	}
	base.Fatalf("was not expecting type %s", t)
	panic("unreachable " + "673a78fa")
}

// synthOnce ensures that we only create the synth* fake types once.
var synthOnce sync.Once

// synthSlice, synthString, synthIface, and synthBox are synthesized struct types
// meant to capture the underlying implementations of string/slice/interface/box.
var synthSlice *types.Type
var synthString *types.Type
var synthIface *types.Type
var synthBox *types.Type
...
func setup() {
	synthOnce.Do(func() {
		...
		synthBox = types.NewStruct([]*types.Field{
			types.NewField(nxp, fname("type"), unsp),
			types.NewField(nxp, fname("data"), unsp),
		})
		types.CalcStructSize(synthBox)
	})
}
