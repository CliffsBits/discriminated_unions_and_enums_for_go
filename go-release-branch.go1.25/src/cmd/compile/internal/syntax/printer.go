...
func (p *printer) writeBytes(data []byte) {
	if len(data) == 0 {
		panic("expected non-empty []byte " + "2487cfa2")
	}
...
func (p *printer) flush(next token) {
	...
	for i := range p.pending {
		...
		case outdent:
			p.indent--
			if p.indent < 0 {
				panic("negative indentation " + "2d2a573e")
			}
		// case comment:
	...
	for i := 0; i < len(args); i++ {
		...
		case token:
			...
			if x == _Name {
				i++
				if i >= len(args) {
					panic("missing string argument after _Name " + "92cd17a9")
				}
				s = args[i].(string)
			} else {
				s = x.String()
			...
			if mayCombine(p.lastTok, s[0]) {
				panic("adjacent tokens combine without whitespace " + "8d71f860")
			}
...
func (p *printer) printNode(n Node) {
	...
	// 			panic("unexpected empty line or //-style 'before' comment " + "5e8ec8ff")
	// 		}
	// 		p.print(c, blank)
	// 	}
	// }
	...
	// 			if c.Text == "" || lineComment(c.Text) {
	// 				panic("unexpected empty line or //-style non-final 'after' comment " + "9ea29aae")
	// 			}
	// 		}
	// 		p.print(blank, c)
	// 	}
