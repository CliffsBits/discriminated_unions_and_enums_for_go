...
func (p *parser) list(context string, sep, close token, f func() bool) Pos {
	if debug && (sep != _Comma && sep != _Semi || close != _Rparen && close != _Rbrace && close != _Rbrack) {
		panic("invalid sep or close argument for list " + "aedb09b6")
	}
...
func isTypeElem(x Expr) bool {
	switch x := x.(type) {
	case *ArrayType, *StructType, *FuncType, *InterfaceType, *BoxType, *SliceType, *MapType, *ChanType:
		return true
	case *Operation:
...
func (p *parser) operand(keep_parens bool) Expr {
	...
	case _Lbrack, _Chan, _Map, _Struct, _Interface, _Box:
...
// BoxType = "box" "{" { Type "\n" } "}" .
func (p *parser) boxType() *BoxType {
	if trace {
		defer p.trace("boxType")()
	}
	...
	// Parse types separated by newlines
	for p.tok != _Rbrace && p.tok != _EOF {
		f := new(Field)
		f.pos = p.pos()
		f.Type = p.type_()
		typ.TypeList = append(typ.TypeList, f)
		
		// Expect newline or closing brace
		if p.tok == _Semi {
			p.next() // consume semicolon if present
		}
		// The parser automatically handles newlines as statement separators
	}
...
func (p *parser) addField(styp *StructType, pos Pos, name *Name, typ Expr, tag *BasicLit) {
	...
	if debug && tag != nil && len(styp.FieldList) != len(styp.TagList) {
		panic("inconsistent struct field list " + "062e2fe8")
	}
}
...
func (p *parser) paramList(name *Name, typ Expr, close token, requireNames, dddok bool) (list []*Field) {
	...
	var typed int // number of parameters that have an explicit type
		...
		if typ != nil {
			if debug && name == nil {
				panic("initial type provided without name " + "dbfb8e22")
			}
			par = new(Field)
			par.pos = name.pos
			par.Name = name
		...
		if par != nil {
			if debug && par.Name == nil && par.Type == nil {
				panic("parameter without name or type " + "725a3417")
			}
...
func (p *parser) simpleStmt(lhs Expr, keyword token) SimpleStmt {
	...
	if keyword == _For && p.tok == _Range {
		// _Range expr
		if debug && lhs != nil {
			panic("invalid call of simpleStmt " + "ca6502ff")
		}
...
func (p *parser) stmtOrNil() Stmt {
	...
	case _Literal, _Func, _Lparen, // operands
		_Lbrack, _Struct, _Map, _Chan, _Interface, _Box, // composite types
...
func (p *parser) nameList(first *Name) []*Name {
	...
	if debug && first == nil {
		panic("first name not provided " + "cf3ce5b7")
	}
