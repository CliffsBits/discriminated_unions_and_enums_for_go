...
func (x *expandState) decomposeAsNecessary(pos src.XPos, b *Block, a, m0 *Value, rc registerCursor) *Value {
	...
	case types.TBOX:
		// Box types have separate operations from interfaces
		mem = x.decomposeOne(pos, b, a, mem, x.typs.Uintptr, OpBTab, &rc)
		pos = pos.WithNotStmt()
		if a.Op == OpBMake {
			data := a.Args[1]
			for data.Op == OpStructMake || data.Op == OpArrayMake1 {
		...
		return x.decomposeOne(pos, b, a, mem, x.typs.BytePtr, OpBData, &rc)
...
func (c *registerCursor) at(t *types.Type, i int) registerCursor {
	...
	panic("Haven't implemented this case yet, do I need to? " + "b6bc482c")
}
...
func (x *expandState) regOffset(t *types.Type, i int) Abi1RO {
	...
	panic("Haven't implemented this case yet, do I need to? " + "cde4490f")
}
