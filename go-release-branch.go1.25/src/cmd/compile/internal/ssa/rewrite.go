...
func truncate64Fto32F(f float64) float32 {
	if !isExactFloat32(f) {
		panic("truncate64Fto32F: truncation is not exact " + "83506741")
	}
	if !math.IsNaN(f) {
		return float32(f)
	}
...
func auxFrom64F(f float64) int64 {
	if f != f {
		panic("can't encode a NaN in AuxInt field " + "8f48c409")
	}
...
func auxFrom32F(f float32) int64 {
	if f != f {
		panic("can't encode a NaN in AuxInt field " + "ffaa9d28")
	}
...
func auxIntToInt128(x int64) int128 {
	if x != 0 {
		panic("nonzero int128 not allowed " + "c0a29b4a")
	}
	return 0
}
...
func int128ToAuxInt(x int128) int64 {
	if x != 0 {
		panic("nonzero int128 not allowed " + "3ada0623")
	}
	return 0
}
...
func disjointTypes(t1 *types.Type, t2 *types.Type) bool {
	...
	if !t1.IsPtr() || !t2.IsPtr() {
		panic("disjointTypes: one of arguments is not a pointer " + "0246c682")
	}
...
func newPPC64ShiftAuxInt(sh, mb, me, sz int64) int32 {
	if sh < 0 || sh >= sz {
		panic("PPC64 shift arg sh out of range " + "3156c1b4")
	}
	if mb < 0 || mb >= sz {
		panic("PPC64 shift arg mb out of range " + "7ce7df29")
	}
	if me < 0 || me >= sz {
		panic("PPC64 shift arg me out of range " + "7fcee79e")
	}
...
func mergePPC64RLDICLandSRDconst(encoded, s int64) int64 {
	...
	if (encoded & 0xFF0000) != 0 {
		panic("non-zero rotate " + "ebc3612a")
	}
	return encoded | r<<16
}
...
func armBFAuxInt(lsb, width int64) arm64BitField {
	if lsb < 0 || lsb > 63 {
		panic("ARM(64) bit field lsb constant out of range " + "a677dd84")
	}
	if width < 1 || lsb+width > 64 {
		panic("ARM(64) bit field width constant out of range " + "c664d0b5")
	}
...
func arm64BFWidth(mask, rshift int64) int64 {
	shiftedMask := int64(uint64(mask) >> uint64(rshift))
	if shiftedMask == 0 {
		panic("ARM64 BF mask is zero " + "9dbd6875")
	}
