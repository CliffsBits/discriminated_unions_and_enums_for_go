...
func decomposeBuiltIn(f *Func) {
	...
	for i, name := range f.Names {
		...
		case t.IsBox():
			typeName, dataName := f.SplitBox(name)
			newNames = maybeAppend2(f, newNames, typeName, dataName)
			for j, v := range f.NamedValues[*name] {
				if v.Op != OpBMake {
					continue
...
func decomposeBoxPhi(v *Value) {
	uintptrType := v.Block.Func.Config.Types.Uintptr
	ptrType := v.Block.Func.Config.Types.BytePtr

	typPtr := v.Block.NewValue0(v.Pos, OpPhi, uintptrType)
	data := v.Block.NewValue0(v.Pos, OpPhi, ptrType)
	for _, a := range v.Args {
		typPtr.AddArg(a.Block.NewValue1(v.Pos, OpBTab, uintptrType, a))
		data.AddArg(a.Block.NewValue1(v.Pos, OpBData, ptrType, a))
	}
	v.reset(OpBMake)
	v.AddArg(typPtr)
	v.AddArg(data)
