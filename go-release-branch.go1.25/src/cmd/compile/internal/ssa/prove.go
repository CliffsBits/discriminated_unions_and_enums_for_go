...
func (ft *factsTable) update(parent *Block, v, w *Value, d domain, r relation) {
	...
	if d == signed || d == unsigned {
		...
		default:
			panic("unknown relation " + "1382800a")
		}
...
//
//	if i >= len(a) {
//	    panic("not in bounds " + "71bc3ddf")
//	}
//	a[i] = 4
//	if i >= len(a) {
//	    panic("not in bounds " + "254a356f")
//	}
//	foo(a[i])
//
...
func prove(f *Func) {
	...
	for _, v := range findIndVar(f) {
		...
		if len(ind.Args) != 2 {
			...
			panic("unexpected induction with too many parents " + "b9122c9c")
		}
		...
		case OpRsh8x8, OpRsh8x16, OpRsh8x32, OpRsh8x64,
			...
			if v.Args[1].isGenericIntConst() && v.Args[1].AuxInt >= bits-1 && ft.isNonNegative(v.Args[0]) {
				...
				case 8:
					v.reset(OpConst8)
				default:
					panic("unexpected integer size " + "cb5ee65c")
				}
				v.AuxInt = 0
