...
func rewriteValuedec(v *Value) bool {
	...
	case OpBData:
		return rewriteValuedec_OpBData(v)
	case OpBMake:
		return rewriteValuedec_OpBMake(v)
	case OpBTab:
		return rewriteValuedec_OpBTab(v)
	case OpComplexImag:
...
func rewriteValuedec_OpBData(v *Value) bool {
	v_0 := v.Args[0]
	b := v.Block
	config := b.Func.Config
	typ := &b.Func.Config.Types
	// match: (BData (BMake _ data))
	// result: data
	for {
		if v_0.Op != OpBMake {
			break
	...
	// match: (BData <t> x)
	// cond: x.Type.IsBox() && x.Op != OpBMake
	// result: (BoxDataPtr <t> x)
	for {
		t := v.Type
		x := v_0
		if !(x.Type.IsBox() && x.Op != OpBMake) {
			break
		}
		v.reset(OpBoxDataPtr)
		v.Type = t
		v.AddArg(x)
		return true
	}
...
func rewriteValuedec_OpBMake(v *Value) bool {
	v_1 := v.Args[1]
	v_0 := v.Args[0]
	// match: (BMake _typ (StructMake val))
	// result: (BMake _typ val)
	for {
		_typ := v_0
...
func rewriteValuedec_OpBTab(v *Value) bool {
	v_0 := v.Args[0]
	b := v.Block
	typ := &b.Func.Config.Types
	// match: (BTab (BMake typPtr _))
	// result: typPtr
	for {
		if v_0.Op != OpBMake {
			break
...
func rewriteValuedec_OpLoad(v *Value) bool {
	...
	// match: (Load <t> ptr mem)
	// cond: t.IsBox()
	// result: (BMake (Load <typ.Uintptr> ptr mem) (Load <typ.BytePtr> (OffPtr <typ.BytePtrPtr> [config.PtrSize] ptr) mem))
	for {
		t := v.Type
		ptr := v_0
		mem := v_1
...
func rewriteValuedec_OpStore(v *Value) bool {
	...
	// match: (Store {t} dst (BMake typPtr data) mem)
	// result: (Store {typ.BytePtr} (OffPtr <typ.BytePtrPtr> [config.PtrSize] dst) data (Store {typ.Uintptr} dst typPtr mem))
	for {
		dst := v_0
		if v_1.Op != OpBMake {
			break
		...
		v1 := b.NewValue0(v.Pos, OpStore, types.TypeMem)
		v1.Aux = typeToAux(typ.Uintptr)
		v1.AddArg3(dst, typPtr, mem)
		v.AddArg3(v0, data, v1)
		return true
	}
	// match: (Store {t} dst src mem)
	// cond: t.IsBox() && src.Op != OpBMake
	// result: (Store {typ.BytePtr} (OffPtr <typ.BytePtrPtr> [config.PtrSize] dst) (BData <typ.BytePtr> src) (Store {typ.Uintptr} dst (BTab <typ.Uintptr> src) mem))
	for {
		t := auxToType(v.Aux)
		dst := v_0
		src := v_1
		mem := v_2
		if !(t.IsBox() && src.Op != OpBMake) {
			break
		}
		v.reset(OpStore)
		v.Aux = typeToAux(typ.BytePtr)
		...
		v0.AuxInt = int64ToAuxInt(config.PtrSize)
		v0.AddArg(dst)
		v1 := b.NewValue0(v.Pos, OpBData, typ.BytePtr)
		v1.AddArg(src)
		v2 := b.NewValue0(v.Pos, OpStore, types.TypeMem)
		v2.Aux = typeToAux(typ.Uintptr)
		v3 := b.NewValue0(v.Pos, OpBTab, typ.Uintptr)
		v3.AddArg(src)
...
func rewriteValuedec_OpStructSelect(v *Value) bool {
	...
	// match: (StructSelect [0] (BData x))
	// result: (BData x)
	for {
		if auxIntToInt64(v.AuxInt) != 0 || v_0.Op != OpBData {
			break
		}
		x := v_0.Args[0]
		v.reset(OpBData)
