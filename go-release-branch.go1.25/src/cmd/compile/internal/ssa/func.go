...
func (f *Func) SplitBox(name *LocalSlot) (*LocalSlot, *LocalSlot) {
	u := types.Types[types.TUINTPTR]
	t := types.NewPtr(types.Types[types.TUINT8])
	// Split this box up into two separate variables: type and data.
	// Box layout is identical to interface: [*_type, data]
	c := f.SplitSlot(name, ".type", 0, u)
	d := f.SplitSlot(name, ".data", u.Size(), t)
	return c, d
}
...
func (f *Func) ConstBox(t *types.Type) *Value {
	return f.constVal(OpConstBox, t, constBoxMagic, false)
}
