...
func rewriteValuegeneric(v *Value) bool {
	...
	case OpBoxDataPtr:
		return rewriteValuegeneric_OpBoxDataPtr(v)
	case OpBoxTypePtr:
		return rewriteValuegeneric_OpBoxTypePtr(v)
	case OpCeil:
	...
	case OpConstBox:
		return rewriteValuegeneric_OpConstBox(v)
	...
	case OpEqBox:
		return rewriteValuegeneric_OpEqBox(v)
	case OpEqInter:
	...
	case OpNeqBox:
		return rewriteValuegeneric_OpNeqBox(v)
	case OpNeqInter:
...
func rewriteValuegeneric_OpBoxDataPtr(v *Value) bool {
	b := v.Block
	typ := &b.Func.Config.Types
	// match: (BoxDataPtr x)
	// result: (ConstNil <typ.BytePtr>)
	for {
		v.reset(OpConstNil)
		v.Type = typ.BytePtr
		return true
...
func rewriteValuegeneric_OpBoxTypePtr(v *Value) bool {
	b := v.Block
	typ := &b.Func.Config.Types
	// match: (BoxTypePtr x)
	// result: (ConstNil <typ.Uintptr>)
	for {
		v.reset(OpConstNil)
		v.Type = typ.Uintptr
		return true
...
func rewriteValuegeneric_OpConstBox(v *Value) bool {
	b := v.Block
	typ := &b.Func.Config.Types
	// match: (ConstBox)
	// result: (BMake (ConstNil <typ.Uintptr>) (ConstNil <typ.BytePtr>))
	for {
		v.reset(OpBMake)
...
func rewriteValuegeneric_OpEqBox(v *Value) bool {
	v_1 := v.Args[1]
	v_0 := v.Args[0]
	b := v.Block
	typ := &b.Func.Config.Types
	// match: (EqBox x y)
	// result: (EqPtr (BTab x) (BTab y))
	for {
		x := v_0
		y := v_1
		v.reset(OpEqPtr)
		v0 := b.NewValue0(v.Pos, OpBTab, typ.Uintptr)
		v0.AddArg(x)
		v1 := b.NewValue0(v.Pos, OpBTab, typ.Uintptr)
		v1.AddArg(y)
		v.AddArg2(v0, v1)
		return true
	}
...
func rewriteValuegeneric_OpNeqBox(v *Value) bool {
	v_1 := v.Args[1]
	v_0 := v.Args[0]
	b := v.Block
	typ := &b.Func.Config.Types
	// match: (NeqBox x y)
	// result: (NeqPtr (BTab x) (BTab y))
	for {
		x := v_0
		y := v_1
		v.reset(OpNeqPtr)
