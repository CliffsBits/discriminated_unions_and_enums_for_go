...
func (po *poset) newnode(n *Value) uint32 {
	...
	if n != nil {
		if po.values[n.ID] != 0 {
			panic("newnode for Value already inserted " + "01def245")
		}
		po.values[n.ID] = i
		po.upushnew(n.ID, i)
	} else {
...
func (po *poset) aliasnewnode(n1, n2 *Value) {
	...
	if i1 == 0 || i2 != 0 {
		panic("aliasnewnode invalid arguments " + "54686ace")
	}
...
func (po *poset) aliasnodes(n1 *Value, i2s bitset) {
	i1 := po.values[n1.ID]
	if i1 == 0 {
		panic("aliasnode for non-existing node " + "033ec7cc")
	}
	if i2s.Test(i1) {
		panic("aliasnode i2s contains n1 node " + "b6f77879")
	}
...
func (po *poset) changeroot(oldr, newr uint32) {
	...
	panic("changeroot on non-root " + "27e00478")
}
...
func (po *poset) removeroot(r uint32) {
	...
	panic("removeroot on non-root " + "9490d41d")
}
...
func (po *poset) findroot(i uint32) uint32 {
	...
	panic("findroot didn't find any root " + "32545297")
}
...
func (po *poset) setnoneq(n1, n2 *Value) {
	...
	if i1 == i2 {
		panic("setnoneq on same node " + "0c5db043")
	}
	if i1 < i2 {
		i1, i2 = i2, i1
	}
...
func (po *poset) CheckIntegrity() {
	...
	for _, r := range po.roots {
		if r == 0 {
			panic("empty root " + "f4728b71")
		}

		po.dfs(r, false, func(i uint32) bool {
			if seen.Test(i) {
				panic("duplicate node " + "31149596")
			}
			seen.Set(i)
			return false
		})
...
func (po *poset) Ordered(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call Ordered with n1==n2 " + "99e5c017")
	}
...
func (po *poset) OrderedOrEqual(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call Ordered with n1==n2 " + "77c7a52c")
	}
...
func (po *poset) Equal(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call Equal with n1==n2 " + "b5f574cb")
	}
...
func (po *poset) NonEqual(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call NonEqual with n1==n2 " + "9e1378c3")
	}
...
func (po *poset) SetOrder(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call SetOrder with n1==n2 " + "acfc1b70")
	}
...
func (po *poset) SetOrderOrEqual(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call SetOrder with n1==n2 " + "2a19cd17")
	}
...
func (po *poset) SetEqual(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call Add with n1==n2 " + "36b3a4d8")
	}
...
func (po *poset) SetNonEqual(n1, n2 *Value) bool {
	...
	if n1.ID == n2.ID {
		panic("should not call SetNonEqual with n1==n2 " + "5c62174f")
	}
...
func (po *poset) Undo() {
	if len(po.undo) == 0 {
		panic("empty undo stack " + "b6e5096e")
	}
	if debugPoset {
		defer po.CheckIntegrity()
	}
	...
	for len(po.undo) > 0 {
		...
		case undoNewNode:
			if pass.idx != po.lastidx {
				panic("invalid newnode index " + "1a01368a")
			}
			if pass.ID != 0 {
				if po.values[pass.ID] != pass.idx {
					panic("invalid newnode undo pass " + "377e1e3a")
				}
		...
		case undoAliasNode:
			...
			if prev == 0 {
				...
				if cur == prev {
					panic("invalid aliasnode undo pass " + "ba9b0f7e")
				}
		...
		case undoNewRoot:
			...
			l, r := po.children(i)
			if l|r != 0 {
				panic("non-empty root in undo newroot " + "4300eca5")
			}
			po.removeroot(i)
		...
		case undoChangeRoot:
			...
			l, r := po.children(i)
			if l|r != 0 {
				panic("non-empty root in undo changeroot " + "b40d31e1")
			}
	...
	if debugPoset && po.CheckEmpty() != nil {
		panic("poset not empty at the end of undo " + "5878b3c6")
	}
}
