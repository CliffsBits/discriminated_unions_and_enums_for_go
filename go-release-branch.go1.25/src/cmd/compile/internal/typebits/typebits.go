...
func set(t *types.Type, off int64, bv bitvec.BitVec, skip bool) {
	...
	case types.TBOX:
		// struct { Type *type; void *data; }
		// Similar to interface{}, box has type info pointer + data pointer
		if off&int64(types.PtrSize-1) != 0 {
			base.Fatalf("typebits.Set: invalid alignment, %v", t)
		}
		// Like interfaces, the first word points to type info in read-only data,
		// so we only mark the second slot (data) as a pointer for GC scanning.
		bv.Set(int32(off/int64(types.PtrSize) + 1)) // pointer in second slot
