...
func walkClosure(clo *ir.ClosureExpr, init *ir.Nodes) ir.Node {
	...
	if x := clo.Prealloc; x != nil {
		if !types.Identical(typ, x.Type()) {
			panic("closure type does not match order's assigned type " + "2acfc018")
		}
		addr.Prealloc = x
		clo.Prealloc = nil
	}
...
func walkMethodValue(n *ir.SelectorExpr, init *ir.Nodes) ir.Node {
	...
	if x := n.Prealloc; x != nil {
		if !types.Identical(typ, x.Type()) {
			panic("partial call type does not match order's assigned type " + "c0147176")
		}
		addr.Prealloc = x
		n.Prealloc = nil
	}
