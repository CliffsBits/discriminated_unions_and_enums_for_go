...
// walkConvBox walks an OCONVBOX node.
func walkConvBox(n *ir.ConvExpr, init *ir.Nodes) ir.Node {
	n.X = walkExpr(n.X, init)

	fromType := n.X.Type()
	toType := n.Type()

	// For now, handle box conversion similar to interface conversion
	// Box types store: [type_tag, data_pointer] like interfaces
	
	// Create the type word (runtime type pointer for fromType)
	typeWord := reflectdata.TypePtrAt(base.Pos, fromType)
	
	// Create the data word (pointer to the actual data) 
	data := dataWord(n, init)
	
	// Create the box value using OMAKEBOX (we'll need to add this operation)
	l := ir.NewBinaryExpr(base.Pos, ir.OMAKEBOX, typeWord, data)
	l.SetType(toType)
	l.SetTypecheck(n.Typecheck())
	return l
}

// typeTagForBoxVariant creates a type tag for storing fromType in toType box
func typeTagForBoxVariant(fromType, toType *types.Type) ir.Node {
	// Box types store actual runtime type pointers, just like interfaces
	// This allows type switches to work by comparing type pointers directly
	boxType := toType.Underlying()
	for i := 0; i < boxType.NumBoxTypes(); i++ {
		if types.Identical(fromType, boxType.BoxVariant(i)) {
			// Return the actual runtime type pointer for fromType
			// This matches interface behavior and enables proper type switches
			return reflectdata.TypePtrAt(base.Pos, fromType)
		}
	}
	base.Fatalf("type %v not found in box %v variants", fromType, toType)
	return nil
}

// boxDataWord returns the data representation for storing a value in a box.
// Box types store data as pointers like interfaces, but with box-specific allocation.
func boxDataWord(conv *ir.ConvExpr, init *ir.Nodes) ir.Node {
	n := conv.X
	fromType := n.Type()
	
	// For simple types, allocate and store a pointer to the value
	// This matches how interface storage works but for boxes
	if fromType.IsInteger() || fromType.IsString() || fromType.IsBoolean() || fromType.IsFloat() {
		// Create a temporary to hold the value
		tmp := typecheck.TempAt(base.Pos, ir.CurFunc, fromType)
		// Assign the value to the temporary
		as := ir.NewAssignStmt(base.Pos, tmp, n)
		init.Append(as)
		// Return pointer to the temporary
		return typecheck.NodAddr(tmp)
	}
	
	// For other types, handle appropriately
	return typecheck.NodAddr(n)
}
