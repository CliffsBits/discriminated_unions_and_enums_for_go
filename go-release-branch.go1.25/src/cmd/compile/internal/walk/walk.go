...
func mayCall(n ir.Node) bool {
	...
	return ir.Any(n, func(n ir.Node) bool {
		...
		case ir.OLITERAL, ir.ONIL, ir.ONAME, ir.OLINKSYMOFFSET, ir.OMETHEXPR,
			...
			ir.OCONVNOP, ir.ODOT,
			ir.OCFUNC, ir.OIDATA, ir.OITAB, ir.OBTAB, ir.OBDATA, ir.OSPTR,
...
// boxData extracts the data from a box type, similar to ifaceData for interfaces.
func boxData(pos src.XPos, n ir.Node, t *types.Type) ir.Node {
	if t.IsInterface() {
		base.Fatalf("boxData interface: %v", t)
	}
	ptr := ir.NewUnaryExpr(pos, ir.OBDATA, n)
	if types.IsDirectIface(t) {
		ptr.SetType(t)
		ptr.SetTypecheck(1)
		return ptr
