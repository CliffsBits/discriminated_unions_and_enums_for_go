...
func walkAssignDotType(n *ir.AssignListStmt, init *ir.Nodes) ir.Node {
	...
	// Check if this is a box type assertion: val, ok := box.(Type)
	if rhs, ok := n.Rhs[0].(*ir.TypeAssertExpr); ok && rhs.X.Type().IsBox() {
		// Handle box type assertion at assignment level
		return walkBoxAssignDotType(n, init)
	}
...
// walkBoxAssignDotType handles two-value box type assertions: val, ok := box.(Type)
func walkBoxAssignDotType(n *ir.AssignListStmt, init *ir.Nodes) ir.Node {
	rhs := n.Rhs[0].(*ir.TypeAssertExpr)
	
	// Store box in temporary variable
	tmp := typecheck.TempAt(base.Pos, ir.CurFunc, rhs.X.Type())
	init.Append(typecheck.Stmt(ir.NewAssignStmt(base.Pos, tmp, rhs.X)))
	
	// Check if types match
	srcTypePtr := ir.NewUnaryExpr(base.Pos, ir.OBTAB, tmp)
	targetType := reflectdata.TypePtrAt(base.Pos, rhs.Type())
	srcTypePtrConverted := ir.NewConvExpr(base.Pos, ir.OCONVNOP, types.Types[types.TUINT8].PtrTo(), srcTypePtr)
	typeEqExpr := ir.NewBinaryExpr(base.Pos, ir.OEQ, srcTypePtrConverted, targetType)
	typeEq := typecheck.Expr(typeEqExpr)
	
	// Extract data if type matches, otherwise use zero value
	value := boxData(base.Pos, tmp, rhs.Type())
	zero := ir.NewZero(base.Pos, rhs.Type())
	
	// Simple approach: just assign the values directly
	// val = type matches ? value : zero
	// ok = type matches
	
	// Assign zero value first, then conditionally override with actual value
	val := n.Lhs[0]
	ok := n.Lhs[1]
	
	// val = zero (default)
	init.Append(typecheck.Stmt(ir.NewAssignStmt(base.Pos, val, zero)))
	// ok = typeEq
	init.Append(typecheck.Stmt(ir.NewAssignStmt(base.Pos, ok, typeEq)))
	
	// if ok { val = value }
	thenBlock := []ir.Node{
		typecheck.Stmt(ir.NewAssignStmt(base.Pos, val, value)),
	}
	ifStmt := ir.NewIfStmt(base.Pos, ok, thenBlock, nil)
	init.Append(typecheck.Stmt(ifStmt))
	
	// Return empty block since all work is done in init
