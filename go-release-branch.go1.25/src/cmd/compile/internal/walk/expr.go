...
func walkExpr1(n ir.Node, init *ir.Nodes) ir.Node {
	...
	case ir.ONOT, ir.ONEG, ir.OPLUS, ir.OBITNOT, ir.OREAL, ir.OIMAG, ir.OSPTR, ir.OITAB, ir.OIDATA, ir.OBTAB, ir.OBDATA:
		n := n.(*ir.UnaryExpr)
	...
	case ir.OMAKEFACE, ir.OMAKEBOX, ir.OAND, ir.OANDNOT, ir.OSUB, ir.OMUL, ir.OADD, ir.OOR, ir.OXOR, ir.OLSH, ir.ORSH,
		ir.OUNSAFEADD:
		n := n.(*ir.BinaryExpr)
		n.X = walkExpr(n.X, init)
...
func walkDotType(n *ir.TypeAssertExpr, init *ir.Nodes) ir.Node {
	...
	// Handle box type assertions - transform them before reaching SSA
	if n.X.Type().IsBox() {
		// Store box in temporary for consistent access (same as type switch)
		tmp := typecheck.TempAt(base.Pos, ir.CurFunc, n.X.Type())
		as := ir.NewAssignStmt(base.Pos, tmp, n.X)
		init.Append(typecheck.Stmt(as))
		
		// Extract box type pointer using OBTAB (same as type switch line 522)
		srcTypePtr := ir.NewUnaryExpr(base.Pos, ir.OBTAB, tmp)
		
		// Get target type pointer (same as type switch line 525)
		targetType := reflectdata.TypePtrAt(base.Pos, n.Type())
		
		// Convert box type pointer to same type as TypePtrAt (same as type switch line 528)
		srcTypePtrConverted := ir.NewConvExpr(base.Pos, ir.OCONVNOP, types.Types[types.TUINT8].PtrTo(), srcTypePtr)
		
		// Create type comparison (same as type switch line 531)
		typeEq := ir.NewBinaryExpr(base.Pos, ir.OEQ, srcTypePtrConverted, targetType)
		typeEq.SetType(types.Types[types.TBOOL])
		
		if n.Op() == ir.ODOTTYPE2 {
			// Two-value form should be handled at the assignment level, not here
			// This should not happen - the two-value form should go through walkAssignDotType
			base.Fatalf("ODOTTYPE2 for box should not reach walkDotType directly: %v", n)
		} else {
			// Single-value form: val := box.(Type) - panic if wrong type
			// Extract data using boxData function (same as type switch)
			value := boxData(base.Pos, tmp, n.Type())
			
			// Check if type matches, panic if not
			panicCall := mkcall("panicnildottype", nil, init, targetType)
			failStmt := ir.NewIfStmt(base.Pos,
				ir.NewUnaryExpr(base.Pos, ir.ONOT, typeEq),
				[]ir.Node{panicCall}, nil)
			init.Append(typecheck.Stmt(failStmt))
