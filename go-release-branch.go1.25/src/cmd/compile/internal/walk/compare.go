...
func walkCompare(n *ir.BinaryExpr, init *ir.Nodes) ir.Node {
	...
	if n.X.Type().IsBox() && n.Y.Type().IsBox() && n.X.Op() != ir.ONIL && n.Y.Op() != ir.ONIL {
		return walkCompareBox(n, init)
	}
...
func walkCompareBox(n *ir.BinaryExpr, init *ir.Nodes) ir.Node {
	// Box comparison: orthogonal to interface comparison
	// Follow similar pattern but use box-specific operations and runtime functions
	
	// Handle potential box conversions similar to interface conversions
	// For now, keep simple - no special OCONVBOX optimization
	n.Y = cheapExpr(n.Y, init)
	n.X = cheapExpr(n.X, init)

	eqtab, eqdata := compare.EqBox(n.X, n.Y)
	var cmp ir.Node
	if n.Op() == ir.OEQ {
		// For equality: btab1 == btab2 && boxeq(btab1, data1, data2)
		cmp = ir.NewLogicalExpr(base.Pos, ir.OANDAND, eqtab, eqdata)
	} else {
		// For inequality: btab1 != btab2 || !boxeq(btab1, data1, data2)
		eqtab.SetOp(ir.ONE)
		cmp = ir.NewLogicalExpr(base.Pos, ir.OOROR, eqtab, ir.NewUnaryExpr(base.Pos, ir.ONOT, eqdata))
	}
	return finishCompare(n, cmp, init)
