...
func (s *exprSwitch) tryJumpTable(cc []exprClause, out *ir.Nodes) bool {
	...
	for _, c := range cc {
		...
		if jmp.Op() != ir.OGOTO || jmp.Label == nil {
			panic("bad switch case body " + "e89ad4d2")
		}
...
func walkSwitchType(sw *ir.SwitchStmt) {
	...
	// Get interface/box descriptor word.
	// For empty interfaces this will be the type.
	// For non-empty interfaces this will be the itab.
	// For boxes this will be the type tag.
	var srcItab *ir.UnaryExpr
	var srcData *ir.UnaryExpr
	if s.srcName.Type().IsBox() {
		// For boxes, ensure we have a value that can be decomposed
		// Create a temporary and store the box to it to ensure it goes through memory
		// This allows the lowering rules to properly decompose OBTAB/OBDATA
		tmp := typecheck.TempAt(base.Pos, ir.CurFunc, s.srcName.Type())
		as := ir.NewAssignStmt(base.Pos, tmp, s.srcName)
		sw.Compiled.Append(typecheck.Stmt(as))
		srcItab = ir.NewUnaryExpr(base.Pos, ir.OBTAB, tmp)
		srcData = ir.NewUnaryExpr(base.Pos, ir.OBDATA, tmp)
	} else {
	...
	flush := func() {
		...
		if len(concreteCases) > 0 {
			...
			for _, c := range concreteCases {
				var as ir.Node
				if s.srcName.Type().IsBox() {
					// Box type: compare type pointers directly
					// Get the type pointer from the box
					srcTypePtr := ir.NewUnaryExpr(c.pos, ir.OBTAB, s.srcName)
					
					// Get the target type's runtime type pointer  
					targetType := reflectdata.TypePtrAt(c.pos, c.typ.Type())
					
					// Convert box type pointer to same type as TypePtrAt (both should be *uint8)
					srcTypePtrConverted := ir.NewConvExpr(c.pos, ir.OCONVNOP, types.Types[types.TUINT8].PtrTo(), srcTypePtr)
					
					// Compare type pointers: ok = (box.typeptr == targetType)
					typeEq := ir.NewBinaryExpr(c.pos, ir.OEQ, srcTypePtrConverted, targetType)
					as = ir.NewAssignStmt(c.pos, s.okName, typeEq)
				} else {
					// Interface type: use regular type assertion
			...
			if len(ncase.List) == 1 {
				...
				if ncase.List[0].Op() == ir.OTYPE { // single compile-time known type
					...
					if t.IsInterface() {
						...
						// This case is a concrete type, extract the value
						if s.srcName.Type().IsBox() {
							// Box type: extract data using OBDATA
							val = boxData(ncase.Pos(), s.srcName, t)
						} else {
							// Interface type: extract data using OIDATA
...
// from an interface's or box's descriptor word (whether a *runtime._type,
// *runtime.itab pointer, or box type tag).
func typeHashFieldOf(pos src.XPos, itab *ir.UnaryExpr) *ir.SelectorExpr {
	if itab.Op() != ir.OITAB && itab.Op() != ir.OBTAB {
		base.Fatalf("expected OITAB or OBTAB, got %v", itab.Op())
	}
	var hashField *types.Field
	if itab.X.Type().IsBox() {
		// Box types - for now only handle non-empty boxes
		// Box type tag is a direct type pointer, uses runtime._type's hash field
		if itab.X.Type().NumBoxTypes() == 0 {
			base.Fatalf("empty box types not yet supported in type switches")
		}
	...
	} else if itab.X.Type().IsEmptyInterface() {
		// Empty interface - runtime._type's hash field
		if rtypeHashField == nil {
			rtypeHashField = runtimeField("hash", rttype.Type.OffsetOf("Hash"), types.Types[types.TUINT32])
		}
		hashField = rtypeHashField
	} else {
		// Non-empty interface - runtime.itab's hash field
