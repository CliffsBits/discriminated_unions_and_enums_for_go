...
func dimportpath(p *types.Pkg) {
	...
	if p == types.LocalPkg && base.Ctxt.Pkgpath == "" {
		panic("missing pkgpath " + "36061b01")
	}
...
var kinds = make([]abi.Kind, 38) // Large enough to hold TBOX at index 37

func init() {
	kinds[types.TINT] = abi.Int
	kinds[types.TUINT] = abi.Uint
	kinds[types.TINT8] = abi.Int8
	kinds[types.TUINT8] = abi.Uint8
	kinds[types.TINT16] = abi.Int16
	kinds[types.TUINT16] = abi.Uint16
	kinds[types.TINT32] = abi.Int32
	kinds[types.TUINT32] = abi.Uint32
	kinds[types.TINT64] = abi.Int64
	kinds[types.TUINT64] = abi.Uint64
	kinds[types.TUINTPTR] = abi.Uintptr
	kinds[types.TFLOAT32] = abi.Float32
	kinds[types.TFLOAT64] = abi.Float64
	kinds[types.TBOOL] = abi.Bool
	kinds[types.TSTRING] = abi.String
	kinds[types.TPTR] = abi.Pointer
	kinds[types.TSTRUCT] = abi.Struct
	kinds[types.TINTER] = abi.Interface
	kinds[types.TCHAN] = abi.Chan
	kinds[types.TMAP] = abi.Map
	kinds[types.TARRAY] = abi.Array
	kinds[types.TSLICE] = abi.Slice
	kinds[types.TFUNC] = abi.Func
	kinds[types.TCOMPLEX64] = abi.Complex64
	kinds[types.TCOMPLEX128] = abi.Complex128
	kinds[types.TUNSAFEPTR] = abi.UnsafePointer
	kinds[types.TBOX] = abi.Box
}
...
func dcommontype(c rttype.Cursor, t *types.Type) {
	...
	if tflag != abi.TFlag(uint8(tflag)) {
		...
		panic("Unexpected change in size of abi.TFlag " + "6e856b0c")
	}
...
func writeITab(lsym *obj.LSym, typ, iface *types.Type, allowNonImplement bool) {
	...
	// Box types should never reach this function - they have their own handling
	if iface.IsBox() {
		base.Fatalf("writeITab should not be called for box types: (%v, %v)", typ, iface)
	}
