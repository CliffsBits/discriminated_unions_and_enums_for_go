...
func (w *writer) boxType(typ *types2.Box) {
	w.Len(typ.NumTypes())
	for i := 0; i < typ.NumTypes(); i++ {
		w.typ(typ.Type(i))
	}
}
...
func (w *writer) exprType(iface types2.Type, typ syntax.Expr) {
	base.Assertf(iface == nil || isInterfaceOrBox(iface), "%v must be nil or an interface/box type", iface)
	...
	if w.Bool(iface != nil && isInterface(iface) && !iface.Underlying().(*types2.Interface).Empty()) {
...
func isInterfaceOrBox(typ types2.Type) bool {
	if _, ok := types2.Unalias(typ).(*types2.TypeParam); ok {
		// typ is a type parameter and may be instantiated as either a
		// concrete or interface type, so the writer can't depend on
		// knowing this.
	...
	case *types2.Interface, *types2.Box:
		return true
	default:
		return false
	}
