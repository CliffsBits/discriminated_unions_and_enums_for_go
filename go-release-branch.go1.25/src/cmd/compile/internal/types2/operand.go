...
func (x *operand) assignableTo(check *Checker, T Type, cause *string) (bool, Code) {
	...
	// T is a box type and V is assignable to one of its constituent types
	if Tb, ok := Tu.(*Box); ok && Tp == nil {
		// nil is always assignable to a box type
		if x.isNil() {
			return true, 0
		}
		for _, elem := range Tb.types {
			if Identical(V, elem) {
				return true, 0
			}
		}
