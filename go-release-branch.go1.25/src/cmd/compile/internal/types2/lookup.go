...
func LookupFieldOrMethod(T Type, addressable bool, pkg *Package, name string) (obj Object, index []int, indirect bool) {
	if T == nil {
		panic("LookupFieldOrMethod on nil type " + "53b711c4")
	}
...
// The underlying type of V must be an interface or box type.
// If the result is false and cause is not nil, *cause describes the error.
// TODO(gri) replace calls to this function with calls to newAssertableTo.
func (check *Checker) assertableTo(V, T Type, cause *string) bool {
	// Handle Box types first
	if Vb, ok := under(V).(*Box); ok {
		// For box types, T must be one of the variant types
		for _, variantType := range Vb.types {
			if Identical(variantType, T) {
				return true
			}
		}
		if cause != nil {
			*cause = "type not present in box"
		}
		return false
	}
...
func deref(typ Type) (Type, bool) {
	if p, _ := Unalias(typ).(*Pointer); p != nil {
		...
		if p.base == nil {
			if debug {
				panic("pointer with nil base type (possibly due to an invalid cyclic declaration) " + "915a67f5")
			}
