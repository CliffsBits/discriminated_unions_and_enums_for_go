...
func (check *Checker) implicitTypeAndValue(x *operand, target Type) (Type, constant.Value, Code) {
	...
	case *Box:
		// Try to find a matching variant type for the untyped value
		for _, variant := range u.types {
			// Avoid infinite recursion - don't try to convert to another box
			if _, isBox := under(variant).(*Box); isBox {
				continue
			}
			if t, v, _ := check.implicitTypeAndValue(x, variant); t != nil {
...
func (check *Checker) exprInternal(T *target, x *operand, e syntax.Expr, hint Type) exprKind {
	...
	case *syntax.AssertExpr:
		...
		case *Interface, *Box:
			// OK - interfaces and box types support type assertions
		default:
			check.errorf(x, InvalidAssert, invalidOp+"%s is not an interface or box type", x)
			goto Error
		}
	...
	case *syntax.ArrayType, *syntax.SliceType, *syntax.StructType, *syntax.FuncType,
		*syntax.InterfaceType, *syntax.BoxType, *syntax.MapType, *syntax.ChanType:
		x.mode = typexpr
		x.typ = check.typ(e)
