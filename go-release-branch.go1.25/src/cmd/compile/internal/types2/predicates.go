...
// IsBox reports whether t is a box type.
func IsBox(t Type) bool {
	_, ok := under(t).(*Box)
	return ok
}
...
func comparableType(T Type, dynamic bool, seen map[Type]bool) *typeError {
	...
	case *Box:
		// Box types are comparable - they use boxeq() runtime function
		return nil
...
func hasNil(t Type) bool {
	...
	case *Box:
		return true // Box types can always be nil
	}
	return false
}
...
func (c *comparer) identical(x, y Type, p *ifacePair) bool {
	...
	case *Box:
		// Two box types are identical if they have the same variant types.
		if y, ok := y.(*Box); ok {
			if x.NumTypes() == y.NumTypes() {
				for i := 0; i < x.NumTypes(); i++ {
					if !c.identical(x.Type(i), y.Type(i), p) {
						return false
					}
				}
				return true
