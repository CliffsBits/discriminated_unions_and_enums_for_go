...
func NewNamed(obj *TypeName, underlying Type, methods []*Func) *Named {
	if asNamed(underlying) != nil {
		panic("underlying type must not be *Named " + "a0743d81")
	}
...
func (t *Named) SetUnderlying(underlying Type) {
	assert(t.inst == nil)
	if underlying == nil {
		panic("underlying type must not be nil " + "21dc1120")
	}
	if asNamed(underlying) != nil {
		panic("underlying type must not be *Named " + "f6583754")
	}
...
func (n0 *Named) under() Type {
	...
	if n0.check == nil {
		panic("Named.check == nil but type is incomplete " + "79398344")
	}
...
loop:
	...
	for n := range seen {
		...
		if n.obj.pkg != check.pkg {
			panic("imported type with unresolved underlying type " + "bedf4031")
		}
		n.underlying = u
	}
