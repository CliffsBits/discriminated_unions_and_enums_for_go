...
func (check *Checker) funcBody(decl *declInfo, name string, sig *Signature, body *syntax.BlockStmt, iota constant.Value) {
	if check.conf.IgnoreFuncBodies {
		panic("function body not ignored " + "3c00970e")
	}
...
L:
	for _, e := range types {
		...
		if check.isNil(e) {
			...
			panic("enable typeHash(T, nil) " + "3ffca0ee")
			// hash = typeHash(T, nil)
		}
		// look for duplicate types
...
func (check *Checker) typeSwitchStmt(inner stmtContext, s *syntax.SwitchStmt, guard *syntax.TypeSwitchGuard) {
	...
	var sx *operand // switch expression against which cases are compared against; nil if invalid
	{
		...
		if x.mode != invalid {
			...
			} else if IsInterface(x.typ) || IsBox(x.typ) {
				sx = &x
			} else {
				check.errorf(&x, InvalidTypeSwitch, "%s is not an interface or box type", &x)
			}
		}
	}
	...
	// Check box exhaustiveness using syntax-based approach to avoid bootstrap issues
	if sx != nil && len(seen) > 0 {
		check.checkBoxExhaustivenessSyntax(s, sx.typ, seen)
	}
...
// checkBoxExhaustivenessSyntax performs exhaustiveness checking using already-computed type information
// to avoid bootstrap-unsafe operations like IsBox() or under() calls during compilation
func (check *Checker) checkBoxExhaustivenessSyntax(s *syntax.SwitchStmt, switchType Type, seen map[Type]syntax.Expr) {
	// Try to access box variants safely - this might fail during bootstrap, so we guard it
	// We'll use deferred error recovery to detect if we're in an unsafe bootstrap context
	defer func() {
		if recover() != nil {
			// If we panic during bootstrap, just return silently
			// This ensures build doesn't hang while still providing checking when safe
		}
	}()
	
	// Attempt to get box variants - this is the only risky operation
	var boxVariants []Type
	if boxType, isBox := switchType.(*Box); isBox {
		boxVariants = boxType.types
	} else {
		// Try underlying type - this might be unsafe during bootstrap
		if underlying := switchType.Underlying(); underlying != switchType {
			if boxType, isBox := underlying.(*Box); isBox {
				boxVariants = boxType.types
			}
		}
	}
	
	// If we couldn't safely determine box variants, skip checking (not a box type)
	if boxVariants == nil {
		return
	}
	
	// Now we know it's a box type - check for default cases first
	for _, clause := range s.Body {
		if clause != nil && clause.Cases == nil {
			check.errorf(clause, InvalidTypeSwitch, "default case not allowed in box type switch (must handle all variants exactly)")
			return
		}
	}
	
	// Find missing variants by comparing box variants against seen cases
	var missing []Type
	for _, variant := range boxVariants {
		if _, covered := seen[variant]; !covered {
	...
	if len(missing) > 0 {
		...
		check.errorf(s, InvalidTypeSwitch, "%s", msg)
	}
}
