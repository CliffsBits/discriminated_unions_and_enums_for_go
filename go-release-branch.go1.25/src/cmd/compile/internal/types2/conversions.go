...
func (check *Checker) conversion(x *operand, T Type) {
	...
	if !ok {
		// Special error message for box types to suggest type assertion  
		if x.typ != nil {
			if u := under(x.typ); u != nil {
				if _, isBox := u.(*Box); isBox {
					check.errorf(x, InvalidConversion, "cannot convert %s to type %s (use type assertion %s.(%s) instead)", x, T, x, T)
					x.mode = invalid
					return
				}
			}
