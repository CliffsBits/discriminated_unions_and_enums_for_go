...
func (d *dwctxt) newtype(gotype loader.Sym) *dwarf.DWDie {
	...
	case abi.Box:
		// Box types are similar to interfaces - pointer-shaped discriminated unions
		// For now, treat them like bare pointer types for DWARF purposes
		// TODO: In the future, could add specific DWARF representation for boxes
		die = d.newdie(&dwtypes, dwarf.DW_ABRV_BARE_PTRTYPE, name)
		typedefdie = d.dotypedef(&dwtypes, name, die)
